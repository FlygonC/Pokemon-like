...
public class MagField : Ability
{
    public class Effect : BattleEffector
    {
        ...
        public override void OnReceiveDamage(BattleManager.BE_ReceiveDamage _event)
        {
            if (_event.battle.AttackBeingUsed is iDealDamage)
            {
                if (_event.target == source && (_event.battle.AttackBeingUsed as iDealDamage).AttackType == AttackType.Power)
                {
                    _event.finalDmgMods.Add(0.75f);
                    Debug.Log(_event.target.creature.Name + "'s Mag Field reduces power damage!");
                }
            }
        }
    }
...
public class Adrenaline : Ability
{
    public class Effect : BattleEffector
    {
        ...
        public override void OnIdle(BattleManager _battlee)
        {
            if (source.notOnCooldown && source.creature.currentEnergy < source.FinalStats.Energy)
            {
                source.energyRestore += (source.creature.Stats.Energy * 0.05f) * 1;
            }
        }
    }
...
public class FastHealer : Ability
{
    public class Effect : BattleEffector
    {
        ...
        /*public override void OnTakeTurn(BattleManager.BE_TakeAction _event)
        {
        ...
        public override void OnIdle(BattleManager _battle)
        {
            if (source.creature.currentHP < source.FinalStats.HP)
            {
                source.HPRestore += (1.0f) * 1;
            }
        }
    }
...
public class Battery : Ability
{
    public class Effect : BattleEffector
    {
        ...
        public override void OnTakeAction(BattleManager.BE_TakeAction _event)
        {
            if (_event.battle.SourceBattler == source)
            {
